...
Phase skills that commit (Code, Simplify, Review, Security, Learn) have both axes. Phase skills that don't commit (Start) only has Continue. Utility skills (Abort, Cleanup) have a single mode value shown under Commit. The `/flow-commit` skill is not configurable — it defaults to auto and can be overridden with `--manual`.
