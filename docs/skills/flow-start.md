...
3. Runs `lib/start-setup.py` — worktree creation, empty commit + push + PR, and state file creation. The user's raw input (including `#N` issue references) is written to `.flow-start-prompt` and passed via `--prompt-file` so it is preserved verbatim in the state file for issue closing at completion
