...
1. **start-init** — acquires start lock, runs version gate and upgrade check, creates early state file via `init-state`. Consults the "Flow In-Progress" label on referenced issues as a pre-lock cross-machine WIP guard; the label apply happens later in `start-workspace` so a failed start-gate or start-workspace leaves no sticky label. (when another start holds the lock, `start-init` blocks on the lock with a bounded ~8-minute internal poll and the skill re-runs the single start-init line on cap-exhaustion until the lock is released)
