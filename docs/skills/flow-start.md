...
When the prompt contains `#N` issue references (e.g., `work on issue #309`), Claude fetches the issue title and derives the branch name from it instead of the prompt words. This produces descriptive names like `organize-settings-allow-list` rather than generic names like `fix-issue-309`. If the issue fetch fails, it falls back to deriving from the prompt words.
