...
| `complete_step` | integer | Current Complete phase step (1-12). Set via `set-timestamp` at each step start. Used by the TUI to show "merging PR - step 8 of 12" and as resume point for CI gate loops |
| `complete_steps_total` | integer | Total number of Complete phase steps (hardcoded 12). Set via `set-timestamp` after phase entry. Used by the TUI for "step N of M" display |
