...
| `framework` | string | `"rails"`, `"python"`, `"ios"`, `"go"`, or `"rust"` — resolved at `/flow-start` time by `resolve_framework()` in `src/commands/init_state.rs`: fresh filesystem detection (via `detect_framework::detect()`) takes precedence when exactly one framework matches the project's marker files; falls back to `.flow.json`'s stored value when detection is ambiguous (zero or multiple matches) or `frameworks_dir()` is unresolvable; defaults to `"rails"` as a last resort only when `.flow.json` also lacks the `framework` key |
