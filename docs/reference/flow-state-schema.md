...
| `complete_step` | integer | Current Complete phase step (1-6). Set by Rust commands at each step boundary. Used by the TUI to show "merging PR - step 5 of 7" and as resume point for CI gate loops |
| `complete_steps_total` | integer | Total number of Complete phase steps. Set by `complete-fast` (5) or `complete-preflight` (7) after phase entry. Used by the TUI for "step N of M" display |
...
| `_stop_instructed` | boolean | Discussion-mode flag. Set to `true` by the Stop hook (`check_discussion_mode`) on the first user interruption during an active flow — the hook blocks and instructs the model to invoke `/flow:flow-note`. On subsequent interruptions the flag is already `true` and the hook allows stop. Cleared by `check_continue` when `_continue_pending` is consumed (atomically in the same `mutate_state` closure) and by `phase_enter()` on phase entry. Transient. |
