...
- **Flow-specific errors** (Steps 2-3: bad config, issue fetch
  failure, duplicate issue) — release the lock before stopping.
  Main is untouched. The next queued flow would succeed.
- **Main-broken errors** (Steps 6-8: CI failure on pristine
  main, dependency breakage) — hold the lock and stop. Main is
  broken. The next queued flow would hit the same failure.
  Releasing cascades the problem.
