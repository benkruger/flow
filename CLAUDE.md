...
`init-state` resolves the `framework` field at flow-start time via `resolve_framework()` in `src/commands/init_state.rs`. When `detect_framework::detect()` finds exactly one framework from the project's marker files (`Cargo.toml`, `pyproject.toml`/`setup.py`/`requirements.txt`, `Gemfile`, `go.mod`, `*.xcodeproj`), that value is written to the state file regardless of `.flow.json`'s stored value. For ambiguous projects (zero or multiple detected frameworks), the helper falls back to `.flow.json` — preserving the prime-time choice for multi-framework repos. When `frameworks_dir()` cannot be resolved or `.flow.json` lacks the `framework` key, the helper defaults to `Framework::Rails`. This protects against stale `.flow.json` framework values (from a prior prime, a QA-mode workflow, or a manual edit) propagating into new state files and breaking downstream framework-aware commands like `bin/flow ci`, `bin/flow test`, `bin/flow build`, `bin/flow lint`, `bin/flow format`, and the Code Review adversarial agent — all of which read `framework` from the state file via `framework_tools::detect_framework_for_project()`.
