...
fn tokens_block_renders_em_dash_when_cost_unknown_but_tokens_grew() {
    ...
    assert!(
        panel.contains("(—)"),
        "em-dash placeholder must appear in cost field when the model is unpriced:\n{}",
        panel
    );
}
