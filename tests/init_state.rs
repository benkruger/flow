...
// --- JSON output parity with Python ---

/// Normalize a state JSON value by replacing timestamps and session_tty
/// with fixed placeholders so structural comparison works across implementations.
fn normalize_state(state: &mut Value) {
    // Replace top-level timestamp
    if state.get("started_at").is_some() {
        state["started_at"] = json!("TIMESTAMP");
    }
    // Replace session_tty
    state["session_tty"] = json!(null);

    // Normalize phase timestamps
    if let Some(phases) = state.get_mut("phases") {
        if let Some(map) = phases.as_object_mut() {
            for (_key, phase) in map.iter_mut() {
                if let Some(obj) = phase.as_object_mut() {
                    for field in ["started_at", "completed_at", "session_started_at"] {
                        if let Some(v) = obj.get(field) {
                            if v.is_string() {
                                obj.insert(field.to_string(), json!("TIMESTAMP"));
                            }
                        }
                    }
                }
...
#[test]
fn rust_output_matches_python_key_order() {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let python_script = std::path::PathBuf::from(manifest_dir)
        .join("lib")
        .join("init-state.py");

    // Skip if Python script doesn't exist (already deleted in later task)
    if !python_script.exists() {
        return;
    }

    // Set up two identical projects
    let py_dir = tempfile::tempdir().unwrap();
    let rs_dir = tempfile::tempdir().unwrap();

    for dir in [py_dir.path(), rs_dir.path()] {
        setup_project(dir, "python", Some(json!({"flow-start": {"continue": "manual"}})));
    }

    // Run Python
    let py_output = Command::new("python3")
        .args([python_script.to_str().unwrap(), "parity test"])
        .current_dir(py_dir.path())
        .output()
        .unwrap();
    assert_eq!(
    ...
    // Run Rust
    let rs_output = run_init_state(rs_dir.path(), &["parity test"]);
    assert_eq!(
        rs_output.status.code(),
        Some(0),
        "Rust failed: {}",
        String::from_utf8_lossy(&rs_output.stderr)
    );

    // Read state files
    let mut py_state = read_state_file(py_dir.path(), "parity-test");
    let mut rs_state = read_state_file(rs_dir.path(), "parity-test");

    // Compare key order at top level
    let py_keys: Vec<&String> = py_state.as_object().unwrap().keys().collect();
    let rs_keys: Vec<&String> = rs_state.as_object().unwrap().keys().collect();
    assert_eq!(py_keys, rs_keys, "Top-level key order mismatch");

    // Compare key order in phases
    let py_phase_keys: Vec<&String> = py_state["phases"].as_object().unwrap().keys().collect();
    let rs_phase_keys: Vec<&String> = rs_state["phases"].as_object().unwrap().keys().collect();
    assert_eq!(py_phase_keys, rs_phase_keys, "Phase key order mismatch");

    // Compare key order in files block
    let py_file_keys: Vec<&String> = py_state["files"].as_object().unwrap().keys().collect();
    let rs_file_keys: Vec<&String> = rs_state["files"].as_object().unwrap().keys().collect();
    assert_eq!(py_file_keys, rs_file_keys, "Files block key order mismatch");

    // Compare key order within each phase
    for phase_key in &py_phase_keys {
        let py_inner: Vec<&String> = py_state["phases"][phase_key.as_str()]
            .as_object()
            .unwrap()
            .keys()
            .collect();
        let rs_inner: Vec<&String> = rs_state["phases"][phase_key.as_str()]
            .as_object()
            .unwrap()
            .keys()
            .collect();
        assert_eq!(
            py_inner, rs_inner,
            "Key order mismatch in phase {}",
            phase_key
        );
    }

    // Normalize and compare full values
    normalize_state(&mut py_state);
    normalize_state(&mut rs_state);
    assert_eq!(py_state, rs_state, "Normalized state mismatch");
}

#[test]
fn rust_output_matches_python_with_auto_skills() {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let python_script = std::path::PathBuf::from(manifest_dir)
        .join("lib")
        .join("init-state.py");
    ...
    for dir in [py_dir.path(), rs_dir.path()] {
        setup_project(dir, "rails", None);
    }

    // Run Python with --auto
    let py_output = Command::new("python3")
        .args([python_script.to_str().unwrap(), "auto parity", "--auto"])
        .current_dir(py_dir.path())
        .output()
        .unwrap();
    assert_eq!(py_output.status.code(), Some(0));

    // Run Rust with --auto
    let rs_output = run_init_state(rs_dir.path(), &["auto parity", "--auto"]);
    assert_eq!(rs_output.status.code(), Some(0));

    let mut py_state = read_state_file(py_dir.path(), "auto-parity");
    let mut rs_state = read_state_file(rs_dir.path(), "auto-parity");

    // Compare skills key order
    let py_skill_keys: Vec<&String> = py_state["skills"].as_object().unwrap().keys().collect();
    let rs_skill_keys: Vec<&String> = rs_state["skills"].as_object().unwrap().keys().collect();
    assert_eq!(py_skill_keys, rs_skill_keys, "Skills key order mismatch");

    normalize_state(&mut py_state);
    normalize_state(&mut rs_state);
    assert_eq!(py_state, rs_state, "Normalized auto-skills state mismatch");
}

#[test]
fn rust_output_matches_python_no_skills() {
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let python_script = std::path::PathBuf::from(manifest_dir)
        .join("lib")
        .join("init-state.py");
    ...
    let py_output = Command::new("python3")
        .args([python_script.to_str().unwrap(), "no skills parity"])
        .current_dir(py_dir.path())
        .output()
        .unwrap();
    assert_eq!(py_output.status.code(), Some(0));

    let rs_output = run_init_state(rs_dir.path(), &["no skills parity"]);
    assert_eq!(rs_output.status.code(), Some(0));

    let mut py_state = read_state_file(py_dir.path(), "no-skills-parity");
    let mut rs_state = read_state_file(rs_dir.path(), "no-skills-parity");
