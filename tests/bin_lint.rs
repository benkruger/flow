//! Tests for `bin/lint` — the FLOW dogfood linter.
...
/// bin/lint must exist and be executable.
#[test]
fn script_is_executable() {
    let script = common::bin_dir().join("lint");
    assert!(script.exists(), "bin/lint must exist");
    let meta = fs::metadata(&script).unwrap();
    assert!(
        meta.permissions().mode() & 0o111 != 0,
        "bin/lint must be executable"
    );
}

/// bin/lint must contain valid bash syntax.
#[test]
...
/// bin/lint invokes `cargo clippy` with `-D warnings`.
#[test]
fn invokes_cargo_clippy_with_deny_warnings() {
    let dir = tempfile::tempdir().unwrap();
    let bin_dir = dir.path().join("bin");
    fs::create_dir_all(&bin_dir).unwrap();

    let real_script = common::bin_dir().join("lint");
    let script_content = fs::read_to_string(&real_script).unwrap();
    let target = bin_dir.join("lint");
    ...
    assert!(
        logged.contains("clippy"),
        "expected cargo clippy, got: {}",
        logged
    );
    assert!(
        logged.contains("-D warnings"),
        "expected -D warnings, got: {}",
        logged
    );
}

/// bin/lint propagates a nonzero exit code from cargo.
#[test]
