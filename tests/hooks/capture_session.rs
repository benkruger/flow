...
/// real path into the new state file. Downstream transcript walkers
/// (`validate-skill` user-only enforcement, `validate-ask-user`
/// carve-outs, the stop hooks, `clear-halt`) then have a
/// transcript_path to scan — if `transcript_path` were null at state
/// init, those gates would have no transcript to read and would fail
/// open.
///
/// Symlink-escape stays closed at every read-time consumer: the
/// transcript walkers in `src/hooks/transcript_walker.rs` re-validate
/// via the canonical wrapper before any `File::open`. Storing a
/// shape-valid path string in the capture file is inert until one of
/// those read-time consumers opens it.
#[test]
