...
spec = importlib.util.spec_from_file_location(
    "create_sub_issue", LIB_DIR / "create-sub-issue.py"
)
sub_issue_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(sub_issue_mod)


def _make_api_router(parent_id=100, child_id=200, link_ok=True,
                     parent_fail=False, child_fail=False):
    ...
    def side_effect(cmd, **kwargs):
        ...
        if call_count["n"] == 1:
            ...
            if parent_fail:
                return subprocess.CompletedProcess(
                    args=cmd, returncode=1, stdout="", stderr="Parent not found",
                )
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout=f"{parent_id}\n", stderr="",
            )
        else:
            # Second call = child
            if child_fail:
                return subprocess.CompletedProcess(
                    args=cmd, returncode=1, stdout="", stderr="Child not found",
                )
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout=f"{child_id}\n", stderr="",
            )
    return side_effect
...
class TestResolveId:
    ...
    def test_happy_path(self):
        ...
        with patch.object(sub_issue_mod.subprocess, "run",
...
class TestCreateSubIssue:
    ...
    def test_parent_id_resolution_fails(self):
        with patch.object(sub_issue_mod.subprocess, "run",
                          side_effect=_make_api_router(parent_fail=True)):
            result, error = sub_issue_mod.create_sub_issue("o/r", 1, 2)
    ...
    def test_child_id_resolution_fails(self):
        with patch.object(sub_issue_mod.subprocess, "run",
                          side_effect=_make_api_router(child_fail=True)):
            result, error = sub_issue_mod.create_sub_issue("o/r", 1, 2)
...
class TestMain:
    ...
    def test_main_success(self, capsys):
        with patch.object(sub_issue_mod.subprocess, "run",
                          side_effect=_make_api_router(100, 200)), \
             patch("sys.argv", ["create-sub-issue.py",
                                "--repo", "o/r",
                                "--parent-number", "1",
                                "--child-number", "2"]):
            sub_issue_mod.main()
    ...
    def test_main_missing_required_args(self):
        with patch("sys.argv", ["create-sub-issue.py", "--repo", "o/r"]), \
