...
fn read_agent_frontmatter(name: &str) -> serde_yaml::Value {
    ...
    assert!(parts.len() >= 3, "{} missing YAML frontmatter delimiters", name);
...
fn phase_skills_2_through_5_have_hard_gate_checking_previous_phase() {
    ...
    for (key, skill) in &ps[1..ps.len() - 1] {
        ...
        assert!(content.contains("<HARD-GATE>"), "Phase {} ({}) has no <HARD-GATE>", key, skill);
        if PHASE_ENTER_PHASES.contains(&key.as_str()) {
            assert!(content.contains("phase-enter"), "Phase {} ({}) HARD-GATE doesn't use phase-enter", key, skill);
        } else {
            let idx = order.iter().position(|k| k == key).unwrap();
            let prev = &order[idx - 1];
            let pat = format!("phases.{}.status", prev);
            assert!(content.contains(&pat), "Phase {} ({}) HARD-GATE doesn't check {}", key, skill, pat);
        }
    }
}
...
fn utility_skills_have_no_phase_gate() {
    ...
    for name in common::utility_skills() {
        ...
        assert!(!re.is_match(&content), "Utility skill '{}' has a phase status check", name);
    }
}
...
fn phase_1_has_no_previous_phase_gate() {
    ...
    let re = Regex::new(r"phases\.[\w-]+\.status").unwrap();
    assert!(!re.is_match(&content), "Phase 1 (start) should not gate on any phase status");
}
...
fn phase_skills_1_through_5_have_done_section_hard_gate() {
    ...
    for (key, skill) in &ps[..ps.len() - 1] {
        ...
        let gates: Vec<String> = re.captures_iter(&content).map(|c| c[1].to_string()).collect();
        let has_continue = gates.iter().any(|g| g.contains("continue=manual") && g.contains("continue=auto"));
        assert!(has_continue, "Phase {} ({}) has no HARD-GATE enforcing continue-mode branching", nums[key], skill);
    }
}
    ...
    for name in common::all_skill_names() {
        ...
        for entry in fs::read_dir(&skill_dir).unwrap().flatten() {
            ...
            for (i, cap) in re.captures_iter(&content).enumerate() {
                let block = &cap[1];
                if placeholder_re.is_match(block) { continue; }
                let stripped = block.trim();
                if !stripped.starts_with('{') && !stripped.starts_with('[') { continue; }
                if block.contains("[...]") || block.contains("...") { continue; }
                assert!(serde_json::from_str::<Value>(block).is_ok(),
                    "Invalid JSON in {}/{} block {}", name, path.file_name().unwrap().to_string_lossy(), i);
            }
        }
    }
}
...
fn flow_references_point_to_existing_skills() {
    ...
    for name in &skills {
        ...
        for cap in re.captures_iter(&content) {
            ...
            if ref_name.contains('<') { continue; }
            assert!(skill_set.contains(ref_name),
                "skills/{}/SKILL.md references /flow:{} but skills/{}/ does not exist", name, ref_name, ref_name);
        }
    }
}
...
fn phase_transitions_follow_sequence() {
    ...
    for i in 0..order.len() - 1 {
        ...
        let skill_name = phases["phases"][key]["command"].as_str().unwrap().split(':').nth(1).unwrap();
        ...
        assert!(content.contains(&pattern),
            "Phase {} ({}) transition should reference Phase {} ({})", nums[key], skill_name, next_num, next_name);
    }
}
...
fn start_uses_ci_fixer_subagent() {
    ...
    assert!(c.contains("ci-fixer"), "flow-start must reference ci-fixer sub-agent");
}

#[test]
fn complete_uses_ci_fixer_subagent() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("ci-fixer"), "flow-complete must reference ci-fixer sub-agent");
}
...
fn code_review_has_six_tenants() {
    ...
    for tenant in &["Architecture", "Simplicity", "Maintainability", "Correctness", "Test coverage", "Documentation"] {
        assert!(c.contains(tenant), "flow-code-review missing tenant '{}'", tenant);
    }
}
...
fn complete_merge_command_no_delete_branch() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("--delete-branch"), "flow-complete merge must not use --delete-branch");
}

#[test]
fn complete_does_not_contain_admin_flag() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("--admin"), "flow-complete must never mention --admin flag");
}

#[test]
fn complete_navigates_to_project_root() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("project root") || c.contains("project_root"),
...
fn assert_agent_exists(filename: &str, required_keys: &[&str]) {
    ...
    for key in required_keys {
        assert!(map.contains_key(&serde_yaml::Value::String(key.to_string())),
            "{} missing '{}' in frontmatter", filename, key);
    }
}

#[test]
fn ci_fixer_agent_exists() { assert_agent_exists("ci-fixer.md", &["name", "model", "maxTurns"]); }
#[test]
fn pre_mortem_agent_exists() { assert_agent_exists("pre-mortem.md", &["name", "model", "maxTurns"]); }
#[test]
fn documentation_agent_exists() { assert_agent_exists("documentation.md", &["name", "model", "maxTurns"]); }
#[test]
fn learn_analyst_agent_exists() { assert_agent_exists("learn-analyst.md", &["name", "model", "maxTurns"]); }
#[test]
fn reviewer_agent_exists() { assert_agent_exists("reviewer.md", &["name", "model", "maxTurns"]); }
#[test]
fn adversarial_agent_exists() { assert_agent_exists("adversarial.md", &["name", "model", "maxTurns"]); }

#[test]
fn code_review_no_onboarding_agent() {
    assert!(!common::agents_dir().join("onboarding.md").exists(), "Tombstone: onboarding agent must not exist");
}

#[test]
fn learn_analyst_agent_has_design_note() {
    let c = common::read_agent("learn-analyst.md");
    assert!(c.contains("Design Note"), "learn-analyst.md must have Design Note section");
}

#[test]
fn learn_no_onboarding_subagent() {
    let c = common::read_skill("flow-learn");
    assert!(!c.contains("onboarding"), "flow-learn must not reference onboarding agent");
}

#[test]
fn learn_uses_learn_analyst_subagent() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("learn-analyst"), "flow-learn must reference learn-analyst sub-agent");
}

#[test]
fn code_review_agents_have_sufficient_max_turns() {
    for agent in &["reviewer.md", "pre-mortem.md", "adversarial.md", "documentation.md"] {
...
fn learn_agents_have_sufficient_max_turns() {
    ...
    let turns = fm["maxTurns"].as_u64().unwrap_or(0);
    assert!(turns >= 25, "learn-analyst.md maxTurns ({}) must be >= 25", turns);
}
...
fn agents_have_reasoning_discipline() {
    for agent in &["pre-mortem.md", "reviewer.md", "adversarial.md"] {
        ...
        assert!(c.contains("Reasoning Discipline") || c.contains("Semi-Formal Reasoning"),
            "{} must have Reasoning Discipline section", agent);
    }
}

#[test]
fn semi_formal_reasoning_rule_exists() {
    let path = common::repo_root().join(".claude").join("rules").join("semi-formal-reasoning.md");
    assert!(path.exists(), "semi-formal-reasoning.md rule file must exist");
    let c = fs::read_to_string(&path).unwrap();
    assert!(c.contains("Premise"), "semi-formal-reasoning.md must contain 'Premise'");
    assert!(c.contains("Trace"), "semi-formal-reasoning.md must contain 'Trace'");
}

#[test]
fn cognitive_isolation_lists_all_context_rich_agents() {
    let path = common::repo_root().join(".claude").join("rules").join("cognitive-isolation.md");
    let c = fs::read_to_string(&path).unwrap();
    assert!(c.contains("reviewer"), "cognitive-isolation.md must list reviewer as context-rich");
    assert!(c.contains("learn-analyst"), "cognitive-isolation.md must list learn-analyst as context-rich");
}
...
fn investigation_agents_no_inline_context() {
    for agent in &["pre-mortem.md", "documentation.md", "adversarial.md"] {
        let c = common::read_agent(agent);
        assert!(!c.contains("CLAUDE.md content:") && !c.contains("Rules content:"),
            "{} must NOT receive inline context (context-sparse design)", agent);
    }
}
...
fn reviewer_inline_context_format_convention() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("CLAUDE.md") || c.contains("claude.md"),
...
fn code_review_no_inline_correctness_review() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("### Correctness Review") && !c.contains("## Correctness Review"),
...
fn code_review_no_inline_security_step() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("### Security Review") && !c.contains("## Security Review"),
...
fn code_review_uses_documentation_subagent() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("documentation"), "Code Review must reference documentation sub-agent");
}

#[test]
fn code_review_step_4_handles_no_findings() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("no findings") || c.contains("No findings") || c.contains("no real findings"),
...
fn code_review_no_step_5() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("### Step 5"), "Tombstone: Step 5 merged into Step 4");
}

#[test]
fn code_review_no_step_6() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("### Step 6"), "Tombstone: Step 6 merged into Step 4");
}
...
fn code_review_steps_have_continuation_directives() {
    ...
    assert!(c.contains("Step 1") && c.contains("Step 2") && c.contains("Step 3"),
...
fn code_review_hard_rules_require_step_continuation() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("## Hard Rules"), "Code Review must have Hard Rules section");
}
...
fn phase_skills_have_tool_restriction_in_hard_rules() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if !content.contains("## Hard Rules") { continue; }
        let re = Regex::new(r"(?s)## Hard Rules\n(.*)").unwrap();
        if let Some(cap) = re.captures(&content) {
            let rules = &cap[1];
            assert!(rules.contains("Bash") || rules.contains("bash"),
                "{} Hard Rules must mention Bash tool restrictions", skill);
        }
    }
}
...
fn phase_skills_have_announce_banner() {
    ...
    for (key, skill) in &ps {
        ...
        assert!(content.contains(&pattern), "Phase {} ({}) missing version in banner", num, skill);
        let phase_pattern = format!("Phase {}", num);
        assert!(content.contains(&phase_pattern), "Phase {} ({}) missing phase number in banner", num, skill);
        assert!(content.contains(name), "Phase {} ({}) missing phase name '{}' in banner", num, skill, name);
    }
}
...
fn phase_skills_have_update_state_section() {
    ...
    for (_, skill) in &ps[..ps.len() - 1] {
        ...
        assert!(content.contains("phase-enter") || content.contains("phase-finalize")
            || content.contains("phase-transition") || content.contains("set-timestamp"),
            "{} should have state update instructions", skill);
    }
}
...
fn phase_skills_use_phase_transition_for_entry() {
    ...
    for (_, skill) in &ps[1..] {
        let content = common::read_skill(skill);
        assert!(content.contains("phase-enter") || content.contains("phase-transition"),
            "{} must use phase entry command", skill);
    }
}
...
fn phase_skills_use_phase_transition_for_completion() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        assert!(content.contains("phase-finalize") || content.contains("phase-transition")
            || content.contains("complete-finalize"),
            "{} must use phase completion command", skill);
    }
}
...
fn phase_skills_no_inline_time_computation() {
    ...
    for (_, skill) in &ps {
        ...
        assert!(!re.is_match(&content),
            "{} must not contain inline time computation patterns", skill);
    }
}
...
fn phase_transition_names_current_phase() {
    ...
    for (key, skill) in &ps {
        ...
        if content.contains("COMPLETE") {
            assert!(content.contains(&pattern) || content.contains(&format!("Phase {}:", num)),
                "{} transition should include 'Phase {}: {}'", skill, num, name);
        }
    }
}
...
fn phase_6_has_soft_gate_not_hard_gate() {
    ...
    assert!(c.contains("<SOFT-GATE>") || c.contains("SOFT-GATE") || c.contains("phase-enter"),
...
fn phase_transitions_have_note_capture_option() {
    ...
    for (_, skill) in &ps[..ps.len() - 1] {
        ...
        if content.contains("AskUserQuestion") {
            assert!(content.contains("correction") || content.contains("learning") || content.contains("note"),
                "{} transition question must offer note-capture option", skill);
        }
    }
}
...
fn phase_1_hard_gate_checks_feature_name() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("Feature name") || c.contains("feature name") || c.contains("arguments"),
...
fn flow_start_surfaces_auto_upgrade() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("auto_upgraded"), "flow-start Step 1 must handle auto_upgraded");
}

#[test]
fn flow_start_documents_flow_in_progress_label_step() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("Flow In-Progress") || c.contains("flow_in_progress"),
...
fn phase_skills_have_logging_section() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        assert!(content.contains("## Logging"), "{} must have ## Logging section", skill);
    }
}
...
fn phase_6_has_delete_state_instructions() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("delete") || c.contains("remove") || c.contains("cleanup"),
...
fn back_navigation_names_match_can_return_to() {
    ...
    for key in &order {
        ...
        if can_return_to.is_empty() { continue; }
        let skill = phases["phases"][key]["command"].as_str().unwrap().split(':').nth(1).unwrap();
        let content = common::read_skill(skill);
        for target in can_return_to {
            ...
            assert!(content.contains(target_name) || content.contains(target_str),
                "{} back navigation should reference {} ({})", skill, target_str, target_name);
        }
    }
}
...
fn can_return_to_targets_are_reachable() {
    ...
    for key in &order {
        ...
        for target in can_return_to {
            ...
            assert!(phases["phases"].get(t).is_some(), "can_return_to target '{}' does not exist in phases", t);
        }
    }
}
...
fn phase_skills_complete_banner_includes_timing() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if content.contains("COMPLETE") {
            assert!(content.contains("<formatted_time>") || content.contains("formatted_time"),
                "{} COMPLETE banner must include formatted_time", skill);
        }
    }
}
    ...
    for name in common::utility_skills() {
        ...
        if content.contains("STARTING") || content.contains("COMPLETE") {
            assert!(content.contains(&format!("v{}", version)),
                "Utility skill {} banners must include version", name);
        }
    }
}
...
fn phase_complete_banners_use_formatted_time() {
    ...
    for (_, skill) in &ps {
        ...
        assert!(!banner_re.is_match(&content),
            "{} COMPLETE banner must use <formatted_time>, not <cumulative_seconds>", skill);
    }
}
...
fn no_skills_use_equals_banners() {
    for name in common::all_skill_names() {
        let content = common::read_skill(&name);
        assert!(!content.contains("============"), "{} should not use old ============ banner pattern", name);
    }
}
...
fn starting_banners_use_light_horizontal() {
    ...
    for (_, skill) in &ps {
        ...
        if content.contains("STARTING") {
            assert!(content.contains("──"), "{} STARTING banner must use ── (light horizontal) borders", skill);
        }
    }
}
...
fn complete_banners_use_heavy_horizontal() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if content.contains("COMPLETE") {
            assert!(content.contains("━━"), "{} COMPLETE banner must use ━━ (heavy horizontal) borders", skill);
        }
    }
}
...
fn paused_banners_use_double_horizontal() {
    ...
    for (_, skill) in &ps {
        ...
        if content.contains("Paused") || content.contains("PAUSED") {
            assert!(content.contains("══"), "{} PAUSED banner must use ══ (double horizontal) borders", skill);
        }
    }
}
...
fn complete_banners_have_check_mark() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if content.contains("COMPLETE") {
            assert!(content.contains("✓"), "{} COMPLETE banner must include ✓ marker", skill);
        }
    }
}
...
fn paused_banners_have_diamond() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if content.contains("Paused") || content.contains("PAUSED") {
            assert!(content.contains("◆"), "{} PAUSED banner must include ◆ marker", skill);
        }
    }
}

#[test]
fn format_status_no_equals_banners() {
    let path = common::repo_root().join("lib").join("format-status.py");
    let c = fs::read_to_string(&path).unwrap();
    assert!(!c.contains("============"), "format-status.py must not use old ============ pattern");
}
...
fn docs_no_equals_banners() {
    ...
    for (rel, content) in &docs {
        assert!(!content.contains("============"), "docs/{} must not use old ============ pattern", rel);
    }
}
...
fn commit_no_auto_manual_flags() {
    ...
    assert!(!c.contains("--auto") && !c.contains("--manual"), "Tombstone: flow-commit has no approval prompt flags");
}

#[test]
fn commit_no_mode_detection() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("dual-mode") && !c.contains("Dual-mode"), "Tombstone: dual-mode detection removed");
}

#[test]
fn commit_no_flow_phases_json() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("flow-phases.json"), "Tombstone: flow-commit must not detect via flow-phases.json");
}

#[test]
fn commit_no_maintainer_mode() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("Maintainer mode") && !c.contains("maintainer mode"), "Tombstone: must not reference Maintainer mode");
}

#[test]
fn commit_no_approval_prompt() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("AskUserQuestion"), "Tombstone: must not contain AskUserQuestion");
}

#[test]
fn commit_no_git_reset_head() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("git reset HEAD"), "Tombstone: must not unstage via git reset HEAD");
}

#[test]
fn commit_no_docs_sync() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("docs sync") && !c.contains("Docs Sync") && !c.contains("docs_sync"),
...
fn reset_guard_requires_main_branch() {
    ...
    assert!(c.contains("main") && c.contains("branch"), "Reset must guard against running outside main branch");
}

#[test]
fn reset_has_inventory_step() {
    let c = common::read_skill("flow-reset");
    assert!(c.contains("inventory") || c.contains("Inventory"), "Reset must inventory artifacts before destroying");
}

#[test]
fn reset_has_confirmation() {
    let c = common::read_skill("flow-reset");
    assert!(c.contains("confirm") || c.contains("Confirm"), "Reset must confirm before destroying");
}

#[test]
fn reset_clears_start_lock_queue() {
    let c = common::read_skill("flow-reset");
    assert!(c.contains("start-queue") || c.contains("lock"), "Reset must clean up start-queue lock directory");
}
...
fn flow_qa_has_setup_check() {
    let c = fs::read_to_string(common::repo_root().join(".claude").join("skills").join("flow-qa").join("SKILL.md")).unwrap();
    assert!(c.contains(".qa-repos") || c.contains("qa-repos"), "QA must check .qa-repos/ for setup status");
}

#[test]
fn flow_qa_has_setup_commands() {
    let c = fs::read_to_string(common::repo_root().join(".claude").join("skills").join("flow-qa").join("SKILL.md")).unwrap();
    assert!(c.contains("prime-setup") || c.contains("prime"), "QA must reference prime-setup");
}
...
fn flow_qa_no_create_issue_step() {
    let c = fs::read_to_string(common::repo_root().join(".claude").join("skills").join("flow-qa").join("SKILL.md")).unwrap();
    assert!(!c.contains("flow-create-issue"), "flow-qa must not reference flow-create-issue");
}
...
fn commit_no_mode_resolution() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("## Mode Resolution"), "Tombstone: dual-mode detection removed from commit");
}

#[test]
fn commit_no_separate_ci_step() {
    let c = common::read_skill("flow-commit");
    assert!(!c.contains("bin/flow ci") && !c.contains("bin/ci"),
...
fn commit_has_commit_format_support() {
    let c = common::read_skill("flow-commit");
    assert!(c.contains("commit_format"), "Commit must support commit_format");
    assert!(c.contains("title-only") || c.contains("full"), "Commit must support format options");
}

#[test]
fn no_skill_invokes_commit_with_auto() {
    for name in common::all_skill_names() {
        if name == "flow-commit" { continue; }
        let content = common::read_skill(&name);
        assert!(!content.contains("flow-commit --auto") && !content.contains("flow:flow-commit --auto"),
            "Tombstone: {} must not pass --auto to flow-commit", name);
    }
}
...
fn release_manual_requires_approval() {
    let c = fs::read_to_string(common::repo_root().join(".claude").join("skills").join("flow-release").join("SKILL.md")).unwrap();
    assert!(c.contains("--manual"), "Release --manual flag must pause for approval");
}
    ...
    for name in common::all_skill_names() {
        ...
        for entry in fs::read_dir(&dir).unwrap().flatten() {
            ...
            if fname != "SKILL.md" && fname.ends_with(".md") {
                panic!("No framework fragment files should exist: {}/{}", name, fname);
            }
        }
    }
}
...
fn learning_has_no_worktree_memory_rescue() {
    let c = common::read_skill("flow-learn");
    assert!(!c.contains("memory rescue") && !c.contains("rescue memory"),
...
fn learning_repo_destinations_use_worktree_path() {
    ...
    if c.contains("CLAUDE.md") || c.contains(".claude/rules/") {
        assert!(!c.contains("project_root/CLAUDE.md") && !c.contains("project_root/.claude"),
...
fn learning_has_no_private_destination_paths() {
    let c = common::read_skill("flow-learn");
    assert!(!c.contains("~/.claude/rules/") && !c.contains("~/.claude/CLAUDE.md"),
...
fn learning_destinations_are_repo_only() {
    ...
    assert!(!c.contains("user-level") || c.contains("never"),
...
fn learning_detects_dangling_async_operations() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("dangling") || c.contains("async") || c.contains("background"),
...
fn learning_edits_rules_directly() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("write-rule") || c.contains("Edit") || c.contains("bin/flow write-rule"),
...
fn learning_files_flow_issues_not_learning() {
    ...
    assert!(c.contains("\"Flow\"") || c.contains("'Flow'") || c.contains("--label") && c.contains("Flow"),
...
fn learn_step3_excludes_flow_process_gaps() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("process gap") || c.contains("Process Gap"),
...
fn code_files_flaky_test_issues() {
    ...
    assert!(c.contains("Flaky Test"), "Code skill CI Gate must file Flaky Test issues");
}

#[test]
fn code_review_files_tech_debt_issues() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("Tech Debt") || c.contains("tech debt"),
...
fn code_review_no_inline_simplify_step() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("simplify:simplify"), "Tombstone: simplify plugin removed");
}

#[test]
fn code_review_files_doc_drift_issues() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("Documentation Drift") || c.contains("documentation drift"),
...
fn skills_record_issues_via_add_issue() {
    for name in common::all_skill_names() {
        ...
        if content.contains("bin/flow issue") {
            assert!(content.contains("add-issue"),
                "{} calls bin/flow issue but must also call add-issue", name);
        }
    }
}
...
fn generic_skills_have_no_framework_conditionals() {
    let phase_names: HashSet<String> = common::phase_order().into_iter().collect();
    let generic = vec!["flow-commit", "flow-config", "flow-status", "flow-note", "flow-reset",
        "flow-abort", "flow-issues", "flow-create-issue", "flow-decompose-project",
        "flow-doc-sync", "flow-orchestrate"];
    for name in generic {
        if !common::skills_dir().join(name).join("SKILL.md").exists() { continue; }
        let content = common::read_skill(name);
        assert!(!content.contains("If Rails") && !content.contains("If Python") && !content.contains("If iOS"),
            "Generic skill {} must not have framework conditionals", name);
    }
}
...
fn configurable_skills_support_both_flags() {
    for name in CONFIGURABLE_SKILLS {
        ...
        assert!(c.contains("--auto"), "{} must mention --auto in Usage", name);
        assert!(c.contains("--manual"), "{} must mention --manual in Usage", name);
    }
}
...
fn configurable_skills_have_mode_resolution() {
    for name in CONFIGURABLE_SKILLS {
        let c = common::read_skill(name);
        assert!(c.contains("## Mode Resolution"), "{} must have Mode Resolution section", name);
    }
}
...
fn mode_resolution_references_config_source() {
    ...
    for name in CONFIGURABLE_SKILLS {
        ...
        if PHASE_ENTER_PHASES.contains(name) {
            assert!(text.contains("phase-enter"), "{} Mode Resolution must reference phase-enter", name);
        } else {
            assert!(text.contains(".flow-states/") || text.contains("state file"),
                "{} Mode Resolution must reference state file", name);
        }
    }
}
...
fn prime_presets_cover_all_configurable_skills() {
    ...
    assert!(blocks.len() >= 3, "Expected at least 3 JSON blocks in flow-prime, found {}", blocks.len());
    for (i, preset) in blocks[..3].iter().enumerate() {
        ...
        for skill in CONFIGURABLE_SKILLS {
            assert!(parsed.get(*skill).is_some(), "'{}' missing from preset {} in flow-prime", skill, i);
        }
    }
}
...
fn configurable_skills_match_phase_order() {
    ...
    assert_eq!(actual, expected, "CONFIGURABLE_SKILLS order must match phase order + abort");
}
...
fn start_no_start_setup_reference() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("start-setup"), "Tombstone: start-setup replaced in PR #904");
}

#[test]
fn start_references_start_init() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("start-init"), "flow-start must reference start-init");
}

#[test]
fn start_references_start_gate() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("start-gate"), "flow-start must reference start-gate");
}

#[test]
fn start_references_start_workspace() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("start-workspace"), "flow-start must reference start-workspace");
}

#[test]
fn start_references_phase_finalize() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("phase-finalize"), "flow-start must reference phase-finalize");
}

#[test]
fn start_no_start_finalize() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("start-finalize"), "Tombstone: start-finalize replaced by phase-finalize in PR #925");
}
...
fn phase_enter_skills_no_action_enter() {
    for name in PHASE_ENTER_PHASES {
        let c = common::read_skill(name);
        assert!(!c.contains("--action enter"), "Tombstone: --action enter replaced by phase-enter in {}", name);
    }
}
...
fn release_complete_banner_confirms_marketplace_update() {
    let c = fs::read_to_string(common::repo_root().join(".claude").join("skills").join("flow-release").join("SKILL.md")).unwrap();
    assert!(c.contains("marketplace"), "Release COMPLETE banner must confirm marketplace update");
}
...
fn start_logging_uses_safe_pattern() {
    ...
    if let Some(cap) = re.captures(&c) {
        let section = &cap[1];
        assert!(section.contains("internally") || section.contains("append_log"),
...
fn logged_phases_use_bin_flow_log() {
    ...
    for (_, skill) in &ps[1..4] {
        ...
        if let Some(cap) = re.captures(&content) {
            let section = &cap[1];
            assert!(section.contains("bin/flow log"), "{} Logging section must use bin/flow log", skill);
        }
    }
}
...
fn plan_dag_capture_is_explicit() {
    ...
    assert!(c.contains("DAG") || c.contains("dag"), "Plan Step 2 must have explicit DAG capture instructions");
}

#[test]
fn learn_step3_requires_output_for_findings() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("finding") || c.contains("Finding"), "Learn Step 3 must require output for findings");
}

#[test]
fn learn_detects_truncated_agent_output() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("truncat") || c.contains("marker"), "Learn must check agent output for expected structure");
}

#[test]
fn anti_patterns_has_inline_output_rule() {
    let path = common::repo_root().join(".claude").join("rules").join("anti-patterns.md");
    let c = fs::read_to_string(&path).unwrap();
    assert!(c.contains("Inline Output"), "Anti-patterns rule must have inline output rule");
}
...
fn phase_state_updates_suppress_output() {
    ...
    for (_, skill) in &ps {
        ...
        if content.contains("set-timestamp") {
            ...
            assert!(content.contains("set-timestamp"), "{} must use set-timestamp for state updates", skill);
        }
    }
}
...
fn phase_skills_have_time_format_instruction() {
    ...
    for (_, skill) in &ps {
        let content = common::read_skill(skill);
        if content.contains("COMPLETE") {
            assert!(content.contains("formatted_time"),
                "{} must include time formatting instructions", skill);
        }
    }
}
...
fn start_truncation_proceeds_without_confirmation() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("Truncation") || c.contains("truncat"),
...
fn start_derives_branch_name_from_prompt() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("branch name") || c.contains("Derived branch") || c.contains("branch"),
...
fn flow_start_no_gh_issue_view_instruction() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("gh issue view"), "Tombstone: removed in PR #741");
}

#[test]
fn flow_start_documents_automatic_issue_branch_naming() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("issue") && c.contains("branch"),
...
fn start_no_manual_step_counter() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("start_step_counter") && !c.contains("step_counter"),
...
fn start_no_explicit_lock_release() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("start-lock --release"), "Tombstone: explicit lock release removed in PR #904");
}
...
fn start_no_old_step_numbering() {
    ...
    assert!(c.contains("### Step 1") || c.contains("## Step 1"),
...
fn start_step1_locked_has_hard_gate() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("locked") && c.contains("<HARD-GATE>"),
...
fn prime_commit_step_enforces_flow_commit_exclusively() {
    let c = common::read_skill("flow-prime");
    assert!(c.contains("flow-commit") || c.contains("flow:flow-commit"),
...
fn prime_step_6_no_git_exclude_option() {
    let c = common::read_skill("flow-prime");
    assert!(!c.contains("git config core.excludes") && !c.contains("--git-exclude"),
...
fn prime_step_6_commits_generated_files() {
    let c = common::read_skill("flow-prime");
    assert!(c.contains("commit") && c.contains("flow-commit"),
...
fn prime_has_commit_format_prompt() {
    let c = common::read_skill("flow-prime");
    assert!(c.contains("commit_format") || c.contains("commit format"),
...
fn code_skill_sets_continue_pending_before_commit() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("_continue_pending"), "Code phase must set _continue_pending before flow-commit");
}

#[test]
fn plan_uses_plan_extract_for_issue_fetch() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("plan-extract"), "Plan must use plan-extract command");
}

#[test]
fn plan_no_direct_gh_issue_view() {
    let c = common::read_skill("flow-plan");
    assert!(!c.contains("gh issue view"), "Tombstone: plan-extract handles issue fetch");
}

#[test]
fn code_has_resume_check() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("Resume Check") || c.contains("## Resume"), "Code must have Resume Check section");
}

#[test]
fn code_has_self_invocation_check() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("Self-Invocation") || c.contains("--continue-step"),
...
fn code_commit_self_invokes() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("flow:flow-code --continue-step"),
...
fn code_commit_records_task() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("code_task"), "Code Commit must record code_task via set-timestamp");
}

#[test]
fn code_skill_uses_single_task_framing() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("single task") || c.contains("only this single task"),
...
fn code_skill_has_atomic_group_handling() {
    let c = common::read_skill("flow-code");
    assert!(c.contains("Atomic") || c.contains("atomic"),
...
fn learn_has_resume_check() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("Resume Check") || c.contains("## Resume"), "Learn must have Resume Check section");
}
...
fn learn_step_4_promotes_permissions() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("promote-permissions"), "Learn Step 4 must call promote-permissions");
}

#[test]
fn learn_step_5_self_invokes() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("flow:flow-learn --continue-step"),
...
fn learn_sets_continue_pending_before_child_skills() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("_continue_pending"), "Learn must set _continue_pending");
}

#[test]
fn learn_steps_record_completion() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("set-timestamp"), "Learn steps must record completion");
}

#[test]
fn learn_skill_sets_steps_total() {
    let c = common::read_skill("flow-learn");
    assert!(c.contains("--steps-total") || c.contains("steps_total"),
...
fn plan_skill_does_not_reference_transcript_path() {
    let c = common::read_skill("flow-plan");
    assert!(!c.contains("transcript_path"), "Plan must not contain transcript_path");
}

#[test]
fn complete_skill_uses_render_pr_body() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("render-pr-body"), "Complete must use render-pr-body");
}

#[test]
fn plan_skill_uses_render_pr_body() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("render-pr-body"), "Plan Step 4 must use render-pr-body");
}

#[test]
fn plan_skill_renders_plan_inline() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("Render") || c.contains("render") || c.contains("inline"),
...
fn complete_done_banner_includes_pr_url() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("pr_url") || c.contains("PR URL") || c.contains("pr url"),
...
fn complete_done_banner_includes_phase_timings() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("timing") || c.contains("Timing") || c.contains("cumulative"),
...
fn complete_done_banner_includes_session_summary() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("summary") || c.contains("Summary"),
...
fn complete_post_merge_references_pr_sections() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("PR body") || c.contains("pr body") || c.contains("PR sections"),
...
fn complete_merged_path_includes_post_merge() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("post-merge") || c.contains("post_merge"),
...
fn complete_uses_format_complete_summary() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("format-complete-summary"), "Complete must reference format-complete-summary");
}

#[test]
fn complete_has_resume_check() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("Resume Check") || c.contains("## Resume"),
...
fn complete_sets_continue_pending_before_commit() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("_continue_pending=commit"), "Complete must set _continue_pending=commit");
}

#[test]
fn complete_commit_points_self_invoke() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("flow:flow-complete --continue-step"),
...
fn complete_no_twelve_steps() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("### Step 12"), "Tombstone: 12-step structure consolidated");
}

#[test]
fn complete_no_steps_total_in_skill() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("complete_steps_total"), "Tombstone: complete_steps_total moved to Rust");
}

#[test]
fn complete_no_simulate_branch() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("--simulate-branch"), "Tombstone: --simulate-branch removed");
}

#[test]
fn complete_uses_complete_fast() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("complete-fast"), "flow-complete must reference complete-fast");
}

#[test]
fn complete_uses_complete_finalize() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("complete-finalize"), "flow-complete must reference complete-finalize");
}

#[test]
fn continue_context_includes_mode_flag() {
    let skills_with_min = [
        ("flow-code", 2), ("flow-code-review", 2), ("flow-complete", 9), ("flow-learn", 2),
    ];
    let re = Regex::new(r#""_continue_context=([^"]+)""#).unwrap();
    for (skill, min_count) in skills_with_min {
        let content = common::read_skill(skill);
        let contexts: Vec<String> = re.captures_iter(&content).map(|c| c[1].to_string()).collect();
        let step_contexts: Vec<&String> = contexts.iter().filter(|c| c.contains("--continue-step")).collect();
        assert!(step_contexts.len() >= min_count,
            "Expected >= {} _continue_context with --continue-step in {}, found {}", min_count, skill, step_contexts.len());
        for ctx in &step_contexts {
            assert!(ctx.contains("--auto") || ctx.contains("--manual"),
                "_continue_context in {} must include --auto or --manual: {}", skill, ctx);
        }
    }
}
...
fn skills_no_substep_markers() {
    ...
    for name in common::all_skill_names() {
        let content = common::read_skill(&name);
        assert!(!bold_re.is_match(&content), "{} contains bold sub-step markers", name);
        assert!(!heading_re.is_match(&content), "{} contains heading sub-step labels", name);
    }
}
...
fn plan_skill_has_dag_step() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("decompose:decompose"), "flow-plan must reference decompose:decompose plugin");
}

#[test]
fn plan_skill_has_dag_resume_check() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("dag") || c.contains("DAG"), "flow-plan must check dag for resume");
}

#[test]
fn plan_skill_has_approval_gate() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("AskUserQuestion"), "flow-plan must use AskUserQuestion for approval gate");
}

#[test]
fn plan_skill_does_not_use_plan_mode() {
    let c = common::read_skill("flow-plan");
    assert!(!c.contains("EnterPlanMode"), "flow-plan must not reference EnterPlanMode");
}
...
fn plan_has_continue_pending_for_decompose() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("_continue_pending"), "Plan must set _continue_pending before decompose");
}

#[test]
fn plan_detects_decomposed_label() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("decomposed") || c.contains("Decomposed"),
...
fn plan_step3_extracts_implementation_plan_for_decomposed() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("Implementation Plan"),
...
fn done_hardgates_read_continue_action() {
    ...
    for (_, skill) in &ps[..ps.len() - 1] {
        let content = common::read_skill(skill);
        assert!(content.contains("continue_action"),
            "{} Done HARD-GATE must read continue_action", skill);
    }
}
    ...
    for (_, skill) in &ps[..ps.len() - 1] {
        ...
        if let Some(last) = gates.last() {
            if last.contains("continue_action") {
                assert!(!last.contains("Read tool") || !last.contains(".flow-states/"),
                    "Tombstone: {} Done HARD-GATE should not re-read state file", skill);
            }
        }
    }
}
    ...
    for (_, skill) in &ps[..ps.len() - 1] {
        ...
        if let Some(last) = gates.last() {
            if last.contains("continue=auto") {
                assert!(last.contains("FINAL") || last.contains("final"),
                    "{} Done auto path must have strengthened language", skill);
            }
        }
    }
}
...
fn plan_skill_has_dag_mode_resolution() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("dag") && c.contains("Mode Resolution"),
...
fn plan_validates_target_file_paths() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("Target Path") || c.contains("target path"),
...
fn plan_verifies_script_behavior_assertions() {
    let c = common::read_skill("flow-plan");
    assert!(c.contains("Script Behavior") || c.contains("script behavior"),
...
fn prime_installs_decompose_plugin() {
    let c = common::read_skill("flow-prime");
    assert!(c.contains("decompose"), "flow-prime must install decompose plugin");
}
...
fn flow_issues_has_work_order_section() {
    ...
    assert!(c.contains("Work Order") || c.contains("work order"), "flow-issues must have Work Order section");
}

#[test]
fn flow_issues_has_wip_detection() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("Flow In-Progress"), "flow-issues must reference 'Flow In-Progress'");
}

#[test]
fn flow_issues_has_decomposed_detection() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("decomposed") || c.contains("Decomposed"), "flow-issues must reference decomposed label");
}

#[test]
fn flow_issues_no_dependency_detection() {
    let c = common::read_skill("flow-issues");
    assert!(!c.contains("Depends on #") && !c.contains("depends on #"),
...
fn flow_issues_has_blocked_label_detection() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("Blocked"), "flow-issues must reference Blocked label");
}

#[test]
fn flow_issues_has_stale_detection() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("stale") || c.contains("Stale"), "flow-issues must have stale issue detection");
}

#[test]
fn flow_issues_has_start_commands() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("flow-start") || c.contains("flow:flow-start"),
...
fn flow_issues_start_commands_include_title() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("title") || c.contains("Title"),
...
fn flow_issues_has_impact_ranking() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("impact") || c.contains("Impact"), "flow-issues must have impact ranking");
}
...
fn flow_issues_has_ready_and_blocked_values() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("Ready") && c.contains("Blocked"), "flow-issues must define Ready and Blocked values");
}

#[test]
fn flow_issues_start_commands_exclude_blocked() {
    let c = common::read_skill("flow-issues");
    assert!(c.contains("blocked") || c.contains("Blocked"),
...
fn flow_start_labels_issues() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("label") || c.contains("Label"), "flow-start must document issue labeling");
}

#[test]
fn flow_complete_removes_labels() {
    let c = common::read_skill("flow-complete");
    assert!(c.contains("label-issues --remove") || c.contains("label-issues") && c.contains("remove"),
...
fn create_issue_has_step_dispatch() {
    ...
    assert!(c.contains("Step Dispatch") || c.contains("step dispatch") || c.contains("--step"),
...
fn create_issue_steps_have_banners() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("STARTING") || c.contains("banner"), "Each step must have banner");
}

#[test]
fn create_issue_steps_1_2_have_ask_user() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("AskUserQuestion"), "Steps 1 and 2 must have AskUserQuestion gates");
}

#[test]
fn create_issue_step_1_self_invokes() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("--step"), "Step 1 must self-invoke with --step flag");
}

#[test]
fn create_issue_has_resume_check() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("Resume") || c.contains("resume"), "flow-create-issue must have Resume Check");
}

#[test]
fn create_issue_no_input_classification() {
    let c = common::read_skill("flow-create-issue");
    assert!(!c.contains("input classification") && !c.contains("Input Classification"),
...
fn create_issue_no_exploration_mode() {
    let c = common::read_skill("flow-create-issue");
    assert!(!c.contains("exploration mode") && !c.contains("Exploration Mode"),
        "Tombstone: removed in PR #677");
}
...
fn create_issue_no_multi_issue_path() {
    let c = common::read_skill("flow-create-issue");
    assert!(!c.contains("multi-issue") && !c.contains("Multi-Issue") && !c.contains("multiple issues"),
        "Tombstone: removed in PR #677");
}
...
fn create_issue_has_conversation_gate() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("cold-start") || c.contains("conversation") || c.contains("context"),
...
fn create_issue_step2_has_implementation_plan_section() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("Implementation Plan"), "Step 2 must produce Implementation Plan");
}

#[test]
fn create_issue_has_repo_routing() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("benkruger/flow") || c.contains("repo"),
...
fn create_issue_skips_repo_selection_in_flow_repo() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("FLOW repo") || c.contains("flow repo") || c.contains("plugin repo"),
...
fn create_issue_step1_has_prior_decompose_detection() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("decompose") || c.contains("prior"),
...
fn create_issue_usage_documents_force_decompose() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("--force-decompose"), "Usage must document --force-decompose flag");
}

#[test]
fn create_issue_step2_redecompose_uses_force_flag() {
    let c = common::read_skill("flow-create-issue");
    assert!(c.contains("--force-decompose"), "Re-decompose must use --force-decompose");
}
...
fn complete_no_force_ci() {
    let c = common::read_skill("flow-complete");
    assert!(!c.contains("--force") || c.contains("--force-decompose"),
...
fn no_flow_continue_skill() {
    assert!(!common::skills_dir().join("flow-continue").exists(),
...
fn no_continue_context_rust_command() {
    ...
    assert!(!src.join("continue_context.rs").exists(),
...
fn code_review_no_two_dot_diff() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("origin/main..HEAD") || c.contains("origin/main...HEAD"),
...
fn learn_no_doc_drift_filing() {
    let c = common::read_skill("flow-learn");
    assert!(!c.contains("Documentation Drift"), "Tombstone: doc drift filing moved to code review");
}
...
fn plan_no_branch_show_current() {
    let c = common::read_skill("flow-plan");
    assert!(!c.contains("git branch --show-current"), "Tombstone: consolidated into porcelain output");
}
...
fn code_review_steps_record_completion() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("set-timestamp"), "Code Review steps must record completion via set-timestamp");
}

#[test]
fn code_review_steps_self_invoke() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("flow:flow-code-review --continue-step"),
...
fn code_review_steps_await_background_agents() {
    ...
    for agent in &["reviewer", "pre-mortem", "adversarial", "documentation"] {
        assert!(c.contains(agent), "Step 2 (Launch) must reference {} agent", agent);
    }
}
...
fn code_review_has_self_invocation_check() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("Self-Invocation"), "Code Review must have Self-Invocation Check section");
}

#[test]
fn code_review_has_bash_bintest_check() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("bin/test"), "Step 1 (Gather) must check bin/test");
}

#[test]
fn start_no_explicit_lock_acquire() {
    let c = common::read_skill("flow-start");
    assert!(!c.contains("start-lock --acquire"), "Tombstone: explicit start-lock acquire removed");
}
...
fn start_files_flaky_test_issues() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("Flaky Test"), "Step 2 (start-gate) must file Flaky Test issues");
}

#[test]
fn start_step_2_has_ci_fix_subagent() {
    let c = common::read_skill("flow-start");
    assert!(c.contains("ci-fixer"), "Step 2 (start-gate) must launch ci-fixer sub-agent");
}
...
fn code_review_step_3_has_triage() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("Triage") || c.contains("triage"),
...
fn code_review_step_2_launches_four_agents() {
    let c = common::read_skill("flow-code-review");
    assert!(c.contains("four") || c.contains("4 ") || (
        c.contains("reviewer") && c.contains("pre-mortem") && c.contains("adversarial") && c.contains("documentation")),
...
fn code_review_no_plugin_step() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("code-review:code-review"), "Tombstone: code-review:code-review plugin removed");
}

#[test]
fn code_review_no_plugin_config_axis() {
    let c = common::read_skill("flow-code-review");
    assert!(!c.contains("code_review_plugin"), "Tombstone: code_review_plugin config removed");
}
...
fn skills_no_repo_tracked_files_at_project_root() {
    ...
    for name in common::all_skill_names() {
        ...
        for para in &paragraphs {
            ...
            if !lower.contains("project root") { continue; }
            for exe in &repo_tracked {
                if para.contains(exe) {
                    violations.push(format!("{}: paragraph mentions both '{}' and 'project root'", name, exe));
                }
            }
        }
    }
...
fn no_exec_in_bash_blocks() {
    ...
    assert!(violations.is_empty(), "Bash blocks must not use exec:\n{}", violations.join("\n"));
}
...
fn prime_presets_keys_match_phase_order() {
    ...
    for (i, block) in blocks[..3.min(blocks.len())].iter().enumerate() {
        ...
        assert_eq!(keys, expected, "Preset {} keys must follow phase order + abort", i);
    }
}
...
fn quadruple_fenced_blocks_use_markdown_and_text() {
    for name in common::all_skill_names() {
        ...
        for cap in re.captures_iter(&content) {
            let lang = &cap[1];
            assert!(lang == "markdown" || lang == "text",
                "{} quadruple-fenced block uses '{}' — must be 'markdown' or 'text'", name, lang);
        }
    }
}
...
fn phase_1_hard_gate_requires_rerun_with_arguments() {
    ...
    if let Some(cap) = re.captures(&c) {
        let gate = &cap[1];
        assert!(gate.contains("re-run") || gate.contains("rerun") || gate.contains("Usage"),
