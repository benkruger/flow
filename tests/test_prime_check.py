...
def test_fails_when_flow_version_mismatch(tmp_path):
    ...
    (tmp_path / ".flow.json").write_text(json.dumps({"flow_version": "0.0.0", "framework": "rails"}))
    result = _run(tmp_path)
    assert result.returncode == 0
    data = json.loads(result.stdout)
    assert data["status"] == "error"
...
def test_fails_when_framework_missing(tmp_path):
    ...
    (tmp_path / ".flow.json").write_text(json.dumps({"flow_version": version}))
    result = _run(tmp_path)
    assert result.returncode == 0
    data = json.loads(result.stdout)
    assert data["status"] == "error"
...
def test_happy_path_returns_ok(tmp_path):
    ...
    version = _current_plugin_version()
    (tmp_path / ".flow.json").write_text(json.dumps({"flow_version": version, "framework": "rails"}))
    result = _run(tmp_path)
    assert result.returncode == 0
    data = json.loads(result.stdout)
...
def test_happy_path_python_framework(tmp_path):
    ...
    version = _current_plugin_version()
    (tmp_path / ".flow.json").write_text(json.dumps({"flow_version": version, "framework": "python"}))
    result = _run(tmp_path)
    assert result.returncode == 0
    data = json.loads(result.stdout)
    assert data["status"] == "ok"
