...
//
// Issue #1382 collapses the FLOW lifecycle from 6 phases to 5 by
// removing Phase 2 (Plan) entirely. The plan now travels inside the
// GitHub issue body wrapped in `<!-- FLOW-PLAN-BEGIN -->` /
// `<!-- FLOW-PLAN-END -->` sentinels, extracted by `bin/flow
// plan-from-issue` at flow-start. The tombstones below catch a
// merge resolution that re-introduces any of the deleted artifacts.

#[test]
fn test_phase_2_plan_skill_directory_must_not_exist() {
    // Tombstone: removed in PR #1383. Plan phase no longer exists.
    let root = common::repo_root();
    let path = root.join("skills/flow-plan");
    assert!(
        !path.exists(),
        "skills/flow-plan/ must not exist — Phase 2 (Plan) was removed in #1382. \
         Plans are now extracted from the issue body via bin/flow plan-from-issue."
    );
}

#[test]
fn test_phase_2_plan_extract_source_must_not_exist() {
    // Tombstone: removed in PR #1383. plan_extract was Phase 2's
    // plan-fetch path; plan-from-issue replaces it.
    let root = common::repo_root();
    assert!(
        !root.join("src/plan_extract.rs").exists(),
        "src/plan_extract.rs must not exist — replaced by src/plan_from_issue.rs in #1382."
    );
}

#[test]
fn test_phase_2_plan_check_source_must_not_exist() {
    // Tombstone: removed in PR #1383. plan_check ran the Phase 2
    // gate scanners; both the runner and every scanner are gone.
    let root = common::repo_root();
    assert!(
        !root.join("src/plan_check.rs").exists(),
        "src/plan_check.rs must not exist — Phase 2 gate scanners were removed in #1382."
    );
}

#[test]
fn test_phase_2_scanner_sources_must_not_exist() {
    // Tombstone: removed in PR #1383. The seven scanners were the
    // plan-prose gate vocabulary; plans now live in issue bodies and
    // do not need prose gates.
    let root = common::repo_root();
    let scanners = [
        "src/scope_enumeration.rs",
        "src/external_input_audit.rs",
        "src/duplicate_test_coverage.rs",
        "src/cli_output_contract_scanner.rs",
        "src/deletion_sweep_scanner.rs",
        "src/tombstone_checklist_scanner.rs",
        "src/verify_references_scanner.rs",
    ];
    for s in scanners {
        assert!(
            !root.join(s).exists(),
            "{} must not exist — Phase 2 scanner removed in #1382.",
            s
        );
    }
}
...
#[test]
fn test_phase_2_doc_files_must_not_exist() {
    // Tombstone: removed in PR #1383. Phase 2 has no doc page.
    let root = common::repo_root();
    assert!(
        !root.join("docs/phases/phase-2-plan.md").exists(),
        "docs/phases/phase-2-plan.md must not exist — Phase 2 was removed in #1382. \
         The current phase-2-code.md describes the renumbered Code phase."
    );
    assert!(
        !root.join("docs/skills/flow-plan.md").exists(),
        "docs/skills/flow-plan.md must not exist — flow-plan skill removed in #1382."
    );
}

#[test]
fn test_phase_2_scanner_rule_files_must_not_exist() {
    // Tombstone: removed in PR #1383. The four scanner rules were
    // tied to the Plan-phase prose gate vocabulary; deleting the
    // scanners deletes the rules they enforced.
    let root = common::repo_root();
    let rules = [
    ...
    for r in rules {
        assert!(
            !root.join(r).exists(),
            "{} must not exist — Phase 2 scanner rule removed in #1382.",
            r
        );
    }
}
...
#[test]
fn test_phase_2_flow_plan_phase_config_entry_must_not_exist() {
    // Tombstone: removed in PR #1383. The phase config still drives
    // PHASE_ORDER, phase_names, and commands; flow-plan no longer
    // appears in any of them.
    let root = common::repo_root();
    let content = std::fs::read_to_string(root.join("flow-phases.json"))
        .expect("flow-phases.json must exist");
    assert!(
        !content.contains("flow-plan"),
        "flow-phases.json must not reference flow-plan — phase removed in #1382."
    );
}

#[test]
fn test_phase_2_phase_flow_plan_variant_must_not_exist_in_state() {
    // Tombstone: removed in PR #1383. The Phase enum exposes only
    // the five surviving phases; FlowPlan is gone.
    let root = common::repo_root();
    let content =
        std::fs::read_to_string(root.join("src/state.rs")).expect("src/state.rs must exist");
    assert!(
        !content.contains("FlowPlan"),
        "src/state.rs must not contain the FlowPlan enum variant — Phase 2 was removed in #1382."
    );
}

#[test]
fn test_phase_2_plan_step_state_field_must_not_exist() {
    // Tombstone: removed in PR #1383. plan_step / plan_steps_total
    // tracked Plan-phase TUI progress; both fields were dropped from
    // FlowState along with Plan.
    let root = common::repo_root();
    let content =
        std::fs::read_to_string(root.join("src/state.rs")).expect("src/state.rs must exist");
    assert!(
        !content.contains("plan_step"),
        "src/state.rs must not declare plan_step — Plan-phase state field removed in #1382."
    );
    assert!(
        !content.contains("plan_steps_total"),
        "src/state.rs must not declare plan_steps_total — Plan-phase state field removed in #1382."
    );
}

#[test]
fn test_phase_2_plan_step_counter_must_not_exist() {
    // Tombstone: removed in PR #1383. STEP_COUNTER_FIELDS is the
    // closed enumeration that gates window snapshot capture; only
    // the four surviving phase counters belong in it.
    let root = common::repo_root();
    let content = std::fs::read_to_string(root.join("src/commands/set_timestamp.rs"))
        .expect("src/commands/set_timestamp.rs must exist");
    assert!(
        !content.contains("\"plan_step\""),
        "src/commands/set_timestamp.rs must not list \"plan_step\" in STEP_COUNTER_FIELDS — \
         Plan-phase step counter removed in #1382."
    );
}
