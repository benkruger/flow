...
is filed. The reviewer audits the drafted plan against the
`.claude/rules/` corpus; on `re-decompose` the plan is re-derived
...
<HARD-GATE>
Do NOT proceed to `Validate + File + Link` when the verdict is
...
Do NOT file the issue. Do NOT close the parent vanilla issue.
Do NOT loop further.
