...
```text
──────────────────────────────────────────────────
  FLOW v0.37.0 — Phase 2: Plan — STARTING
──────────────────────────────────────────────────
```
````
...
is inside the repo working tree (the current working directory). Files
outside the repo — such as paths starting with `~/`, `/Users/`, or any
absolute path not under the working directory — are not tracked by git.
Changes to those files will not appear in `git status` or the PR diff.

If a target path is outside the repo and the prompt or issue body
contains keywords like "repo", "version-controlled", "shared",
"committed", "tracked", or "project", flag the ambiguity in the plan's
Risks section and default to the repo-local equivalent path (e.g.
`.claude/rules/` instead of `~/.claude/rules/`).

If the prompt does not contain repo-related keywords, the out-of-repo
path may be intentional. Still note it in the Risks section so the user
is aware, but do not override the target.
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ✓ FLOW v0.37.0 — Phase 2: Plan — COMPLETE (<formatted_time>)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
````
