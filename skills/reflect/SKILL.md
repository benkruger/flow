...
```bash
date -u +"%Y-%m-%dT%H:%M:%SZ [Phase 7] Step X — desc — START" >> /tmp/flow-<branch>.log; COMMAND; EC=$?; date -u +"%Y-%m-%dT%H:%M:%SZ [Phase 7] Step X — desc — DONE (exit $EC)" >> /tmp/flow-<branch>.log; exit $EC
```
