...
```bash
COMMAND; EC=$?; date -u +"%Y-%m-%dT%H:%M:%SZ [Phase 4] Step X — desc (exit $EC)" >> .claude/flow-states/<branch>.log; exit $EC
```

Get `<branch>` from the state file. The gap between consecutive entries
is Claude's processing + command time.
